...
import { deliveryMigrationSql } from './telegram-delivery.ts';
...
export async function migrateOutreach(pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (current.rows.length && ![1, 2].includes(current.rows[0].version)) throw new Error('UNSUPPORTED_OUTREACH_SCHEMA');
