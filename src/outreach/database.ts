...
export async function migrateOutreach(pool: Pool): Promise<void> {
  ...
  try {
    ...
    if (current.rows.length && ![1, 2, 3, 4, 5].includes(current.rows[0].version)) throw new Error('UNSUPPORTED_OUTREACH_SCHEMA');
    ...
    if (!current.rows.length || current.rows[0].version < 4) {
      await client.query(coverTransferMigrationSql);
      await client.query('UPDATE outreach_schema_version SET version = 4 WHERE singleton = true');
    }
    if (!current.rows.length || current.rows[0].version < 5) {
      await client.query(coverCaptionMigrationSql);
      await client.query('UPDATE outreach_schema_version SET version = 5 WHERE singleton = true');
    }
