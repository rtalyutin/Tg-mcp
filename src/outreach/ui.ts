...
type Card = { id: string; kind: string; name: string; website: string | null; version: number;
  ...
  possible_matches?: Array<Record<string, unknown>>; history?: Array<Record<string, unknown>> };
...
document.addEventListener('submit', async event => {
  ...
  try {
    ...
    await request(path, body); feedback('Сохранено'); await pace(); location.assign(destination);
  } catch (error) {
